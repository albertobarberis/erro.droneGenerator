...
function moveDot(currentDot, x, y){
    let indexToDelete = arrayOfDots.indexOf(currentDot); // fetch the index of the Object
    if(indexToDelete == -1) return;
    currentDot.move(x, y);
}
...
class Dot{
    ...
    move(paramX,paramY){
        ...
        this.mainOsc.frequency.value = this.freq; // set the frequency
        this.mainOscGain.gain.setTargetAtTime(this.amp, audioContext.currentTime, ATTACK_TIME/4); 
        this.AMoscGain.gain.setTargetAtTime(this.amp, audioContext.currentTime, ATTACK_TIME/4);
    }

    setRandom(d){
        this.randomizeFlag = !this.randomizeFlag;
        this.randomSize = map(d, 0, this.dimension/2, 0.5, 20);;
    }
    ...
    randomize(){
        ...
        this.x = this.x + this.randomSize*random(0,1)*sigX;
        this.y = this.y + this.randomSize*random(0,1)*sigY;
...
function mousePressed() { // functtion called if the mouse is pressed
    ...
    if(mouseY < 0 || keyIsDown(76) || keyIsDown(77)) return; // exit if the mouse is out of the canvas OR the user is pressing "L"
...
function mouseDragged() { // function called if the mouse is dragged (for the FM synthesis)
     // exit from the function if is not pressed the key 'L'
     if(!(keyIsDown(76)||keyIsDown(77))) return;
    ...
    for(let i=0; i<arrayOfDots.length; i++){
        ...
        if(keyIsDown(76)){
            ...
            if(d < currentDot.dimension/2){ // if pressing on an existing Dot (and key 'L' is not down)
                console.log("MOV ");
                moveDot(currentDot, mouseX, mouseY); // delete the Dot
            }
...
function draw(){ // this is the draw function of the p5.js called to draw on the canvas
    ...
    fill('rgba(0,255,0,0.3)'); // the color of the Dot: red with alpha
    stroke(0, 255, 0);
    ...
    for(let i=0; i<arrayOfDots.length; i++){ // draw each Dot in the array
        ...
        if(currentDot.randomizeFlag){
            currentDot.randomize();
        }
